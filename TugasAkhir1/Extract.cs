...
namespace TugasAkhir1
{
    public class Extract
    {
        ...
        public static double[] BaumWelchDetectionInLH_2(double[,] coeffs, Image watermarkedImage, int NumOfScale2, int NumOfTrees, List<int> PNSeq, string rgb,string subband, double embed_constant)
        {
            ...
            // Convert Matrix of Watermark into List of 5-nodes tree get from LH,HH,and HL.
            double[][] listOfTrees = GetSequenceParameter(coeffs, watermarkedImage, NumOfTrees, rgb).Item1;

            // Get the trees of watermark where the actual watermark was embedded
            double[][] WaveletTrees = TreeOfWatermark2(listOfTrees, NumOfTrees, subband);

            double[][] watermarkPermutation = GetSequenceParameter(coeffs, watermarkedImage, NumOfTrees, rgb).Item2;
            double[,] hvs = GetSequenceParameter(coeffs, watermarkedImage, NumOfTrees, rgb).Item3;
        ...
        public static double[] BaumWelchDetectionInLH_22(double[,] coeffs, Image watermarkedImage, int NumOfScale2, int NumOfTrees, List<int> PNSeq, string rgb, string subband, double embed_constant)
        {
            ...
            double[][] watermarkPermutation = GetSequenceParameter(coeffs, watermarkedImage, NumOfTrees, rgb).Item2;
            double[,] hvs = EdgeBasedHVS(watermarkedImage, rgb);//GetSequenceParameter(coeffs, watermarkedImage, NumOfTrees, rgb).Item3;
        ...
        #region Detection using Baum-Welch Learning Parameter in RGB
        ...
        public static Tuple<double[][], double[][], double[,]> GetSequenceParameter(double[,] coeffs, Image watermarkedImage, int NumOfTree, string rgb)
        {
            ...
            if(rgb == "red")
            {
                double[,] pixels = ImageProcessing.ConvertToMatrix2(new Bitmap(watermarkedImage)).Item1;
                Bitmap EdgeImage = ImageProcessing.LaplaceEdge(new Bitmap(watermarkedImage));
                double[,] pixels2 = ImageProcessing.ConvertToMatrix2(EdgeImage).Item1;
                double[,] Hvs = Embed.AdaptiveHVS(coeffs, pixels, pixels2, NumOfTree);
                double[,] listOfHvs = Haar.ListOfCoeffs(Hvs);
            ...
            {
                double[,] pixels = ImageProcessing.ConvertToMatrix2(new Bitmap(watermarkedImage)).Item3;
                Bitmap EdgeImage = ImageProcessing.LaplaceEdge(new Bitmap(watermarkedImage));
                double[,] pixels2 = ImageProcessing.ConvertToMatrix2(EdgeImage).Item3;
                double[,] Hvs = Embed.AdaptiveHVS(coeffs, pixels, pixels2, NumOfTree);
                double[,] listOfHvs = Haar.ListOfCoeffs(Hvs);
        ...
        public static double[,] EdgeBasedHVS(Image decomposedImage, string rgb)
        {
            if(rgb == "red")
            {
                double[,] EdgeHVS = Embed.EdgeBasedHVS(new Bitmap(decomposedImage)).Item1;
                double[,] listOfHvs = Haar.ListOfCoeffs(EdgeHVS);
                return listOfHvs;
            }
            else if (rgb == "green")
            {
                double[,] EdgeHVS = Embed.EdgeBasedHVS(new Bitmap(decomposedImage)).Item2;
                double[,] listOfHvs = Haar.ListOfCoeffs(EdgeHVS);
                return listOfHvs;
            }
            else
            {
                double[,] EdgeHVS = Embed.EdgeBasedHVS(new Bitmap(decomposedImage)).Item3;
                double[,] listOfHvs = Haar.ListOfCoeffs(EdgeHVS);
                return listOfHvs;
            }
        ...
        public static Tuple<double[][], double[][], double[,]> GetSequenceParameter2(double[,] coeffs, Image watermarkedImage, int NumOfTree, string rgb)
        {
            if (rgb == "red")
            {
                // Trees of wavelet coefficients
        ...
        public static double[,] CalculateVi2(int k, double[][] pattern, double[,] listOfHVS, double embed_constant)
        {
            ...
            for (int i = 0; i < Vi.GetLength(0); i++)
            {
                for (int j = 0; j < Vi.GetLength(1); j++)
                {
                    ...
                    Vi[i, j] = pattern[i][j] * listOfHVS[k, j] * embed_constant;//0.6;
